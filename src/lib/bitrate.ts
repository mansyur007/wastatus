...
export function bitrateForTarget(targetMB: number, durationSec: number, audioKbps: number): number {
  const totalKbits = (targetMB * 1024 * 1024 * 8) / 1000
  const videoKbps = Math.floor((totalKbits * OVERHEAD) / durationSec - audioKbps)
  return Math.max(MIN_VIDEO_KBPS, videoKbps)
}
...
export function effectiveVideoBitrate(s: Settings, meta?: MediaInfo): number {
  if (s.encodingMode === 'size') {
    ...
    return bitrateForTarget(s.targetSizeMB, encodeDuration(s), audioKbps(s, meta))
  }
  // In CRF mode the bitrate field acts as a max-rate cap, so it bounds the estimate.
  const heuristic = estimateCrfBitrate(s, meta)
  return s.videoBitrate > 0 ? Math.min(heuristic, s.videoBitrate) : heuristic
}
